    ...
    def _generate_markdown_report(self):
        ...
        if successful_results:
            ...
            for i, (model_name, result) in enumerate(sorted_results):
                ...
                content.append(
                    f"| **{model_name}** | {duration:.1f} | **{wer_str}** | {rt_str} | {framework} | {status} |\n"
                )
        ...
        if successful_results:
            ...
            if self.reference_text:
                ...
                content.append(
                    f"- **WER: {best_accuracy[1].get('wer', 'N/A'):.3f}** ({best_accuracy[1].get('wer', 0) * 100:.1f}% error rate)\n"
                )
                content.append(f"- Time: {best_accuracy[1]['duration']:.1f}s\n\n")

                content.append(f"### ⚡ **Speed Winner: {best_speed[0]}**\n")
                content.append(
                    f"- **{best_speed[1]['duration']:.1f} seconds** ({best_speed[1]['duration'] / 30:.1f}x real-time)\n"
                )
