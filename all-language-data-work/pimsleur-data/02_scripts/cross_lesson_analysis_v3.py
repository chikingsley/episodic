        ...
        with db.conn.cursor() as cur:
            ...
            with alive_bar(len(utterance_data), title="Processing patterns", force_tty=True) as bar:
                for text, lesson_num, count in utterance_data:
                    ...
                    pattern_data[normalized_text]["original_text"] = (
                        text  # Keep one original for display
                    )
                    bar()
            ...
            with alive_bar(len(pattern_data), title="Calculating metrics", force_tty=True) as bar:
                for normalized_text, data in pattern_data.items():
                    ...
                    persistence_score = calculate_persistence_score(
                        lesson_counts, total_lessons_covered
                    )
            ...
            with alive_bar(
                len(patterns_to_insert), title="Inserting patterns", force_tty=True
            ) as bar:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cross-Lesson Utterance Pattern Analysis V3")
    parser.add_argument(
        "--lessons",
        nargs="+",
        type=int,
