...
### PATTERN 028: Politeness_Upgrade

**When to use:** Introducing a more polite alternative after teaching the basic form

```markdown
[NARRATOR]: As in English, it would be too abrupt to simply say [BASIC_FORM].
So let's learn a more polite way of [FUNCTION].
[TEACH_POLITE_FORM]
```

**Example:** Teaching "Je voudrais" after the learner already knows "Je veux"

---

### PATTERN 029: Pronunciation_Rule_Observation

**When to use:** Drawing the learner's attention to a systematic pronunciation phenomenon

```markdown
[NARRATOR]: Notice how [PHENOMENON].
[EXAMPLE_1]: [PRONUNCIATION_1]
[EXAMPLE_2]: [PRONUNCIATION_2]
[PRACTICE_EXAMPLES]
```

**Example:** Highlighting liaison between "vous" and vowel‑initial words, e.g., "vous avez" → /vu.z‿a.ve/

---

### PATTERN 030: Decremental_Contradiction_Exercise

**When to use:** Learner must systematically decrease the number or amount given by the speaker

```markdown
[SETUP]: For every [ITEM] mentioned, correct by [SYSTEMATIC_CHANGE]
[EXAMPLE]: She says 13 → You say 12
[PRACTICE_SEQUENCE]
```

**Example:** Price haggling where every quoted price is reduced by one unit

---

### PATTERN 031: Bilingual_Listen_Repeat_Cue

**When to use:** A short imperative cue—often in French, sometimes echoed in English—that signals the upcoming listen‑and‑repeat segment. It usually appears as a standalone line (`Écoutez et répétez.` / `Listen and repeat.`) immediately before a drill or backward‑buildup.
...
These lines do **not** introduce new lexical content; they just prime the learner.  
They frequently embed inside PATTERN 004, 005, 022, or 023, but we flag them separately so every transcript line has a home.

---

### PATTERN 032: Lesson_Preamble

**When to use:** The housekeeping intro at the very start of nearly every unit: title, unit number, mention of Speak & Read booklet, etc. Pedagogically neutral but useful for generating faithful reconstructions of Pimsleur‑style lessons.

```markdown
[NARRATOR]: This is Unit [NUMBER] of Pimsleur's Speak and Read Essential [LANGUAGE] 1.
[FRENCH_NARRATOR]: [OPTIONAL_BRIEF_STATEMENT]
```
