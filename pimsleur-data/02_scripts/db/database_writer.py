...
class DatabaseWriter:
    ...
    def __init__(self):
        ...
        print(f"Connected to database for writing")
    ...
    def ensure_lesson_exists(self, lesson_number: int) -> int:
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT id FROM lessons WHERE lesson_number = %s",
                (lesson_number,)
            )
            result = cur.fetchone()
            ...
            if result:
                ...
                print(f"Created lesson {lesson_number} with ID {lesson_id}")
    ...
    def write_utterances(self, utterances: List[UtteranceRecord]) -> int:
        ...
        print(f"Writing {len(utterances)} utterances to database...")
        ...
        with self.conn.cursor() as cur:
            ...
            lesson_ids = set(self.ensure_lesson_exists(u.lesson_number) for u in utterances)
            ...
            print(f"✅ Wrote {len(db_records)} utterances to database")
    ...
    def ensure_phrase_exists(self, phrase_text: str) -> int:
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT id FROM phrases WHERE text = %s",
                (phrase_text,)
            )
            result = cur.fetchone()
    ...
    def write_phrases(self, phrases: List[PhraseRecord]) -> int:
        ...
        print(f"Writing {len(phrases)} phrases to database...")
    ...
    def update_cognitive_load(self, lesson_number: int, metrics: Dict[str, Any]):
        ...
        with self.conn.cursor() as cur:
            ...
            print(f"✅ Updated cognitive load for lesson {lesson_number}")
