...
def calculate_lifecycle_stage(lesson_counts: Dict[int, int], first_lesson: int, last_lesson: int) -> str:
...
def extract_templates_from_patterns(elements: List[PedagogicalElement]) -> Dict[str, List[PedagogicalElement]]:
...
def analyze_persistent_patterns(min_lessons: int = 3, max_utterances: int = None):
    ...
    with DatabaseWriter() as db:
        with db.conn.cursor() as cur:
            ...
            print(f"🔍 Found {len(persistent_patterns)} persistent patterns (appearing in {min_lessons}+ lessons)")
            ...
            with alive_bar(
                ...
                for row in persistent_patterns:
                    text, first_lesson, last_lesson, total_lessons, lesson_counts_json, persistence_score, pattern_type = row
...
def generate_analysis_report(elements: List[PedagogicalElement]):
    ...
    for tier, tier_elements in sorted(tier_groups.items()):
        ...
        for i, elem in enumerate(tier_elements[:5]):
            lessons_str = ", ".join([f"L{k}:{v}" for k, v in sorted(elem.lesson_distribution.items())])
...
def export_to_csv(elements: List[PedagogicalElement]):
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        ...
        sorted_elements = sorted(elements, key=lambda x: (-x.total_lessons, min(x.lesson_distribution.keys())))
...
def save_analysis_results(elements: List[PedagogicalElement], templates: Dict[str, List[PedagogicalElement]]):
    ...
    # Export to CSV first
    csv_file = export_to_csv(elements)
    ...
    print(f"🗄️  Data saved to pattern_analysis table")


def main():
    """Main function for persistent pattern analysis."""
    parser = argparse.ArgumentParser(
        description="Analyze Persistent Utterance Patterns with LLM"
    )
    parser.add_argument(
        "--min-lessons",
        type=int,
