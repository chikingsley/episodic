...
- Automatic Speech Recognition (ASR): Transcribes user's voice to text. This is your initial point of contact with user input.
- Dialogue Manager (DM): The brain of the operation. Uses ASR output, current game state, and pedagogical goals to decide the NPC's next action and response.
- Narrative & Content System (Inky/Nodes): The authored structure that defines the story beats, possible turns, and consequences. The DM routes through this.
- Text-to-Speech (TTS): Generates the NPC's spoken responses.
- Pronunciation & Fluency Assessment Module: Provides specific feedback on pronunciation and rhythm.
...
## Integrating with Inky (Narrative Structure)
...
- Pronunciation Feedback - SpeechAce API
- Goal Orientation and Consequences
